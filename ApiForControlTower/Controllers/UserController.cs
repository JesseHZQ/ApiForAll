...
namespace ApiForControlTower.Controllers
{
    public class UserController : ApiController
    {
        ...
        {
            string sql = "SELECT * FROM [CT_User] ORDER BY UserName";
        ...
        {
            string sql = "UPDATE [CT_User] SET UserName = @UserName, Password= @Password, LevelNum = @LevelNum, Shift = @Shift, Skills = @Skills, Phone = @Phone, Email = @Email, BackUpId = @BackUpId WHERE UserId = @UserId";
        ...
        public int AddUser(User user)
        {
            ...
            if (u == null)
            {
                string sql = "INSERT INTO [CT_User] (UserId, UserName, Password, LevelNum, Shift, Skills, Phone, Email, BackUpId) VALUES (@UserId, @UserName, @Password, @LevelNum, @Shift, @Skills, @Phone, @Email, @BackUpId)";
                return conn.Execute(sql, user);
            } else
            {
                return 0;
        ...
        {
            string sql = "DELETE FROM [CT_User] WHERE UserId = @UserId";
            return conn.Execute(sql, user);
        }
    }
}
